...
import { Blip, FlagType, Item, Point } from "../../model/model";
import Link from "../Link/Link";
import { ChangedBlip, DefaultBlip, RecomendedBlip, NewBlip } from "../Chart/BlipShapes";
...
function generateCoordinates(
  ...
  const quadrant_shift_x = [0, 400, 0, 400];
  const quadrant_shift_y = [0, 0, 400, 400];

  return {
    x: xScale(Math.cos(randomDegree + shift) * radius) - quadrant_shift_x[blip.quadrantPosition - 1],
    y: yScale(Math.sin(randomDegree + shift) * radius) - quadrant_shift_y[blip.quadrantPosition - 1],
  };
}
...
function renderBlip(
  ...
  switch (blip.flag) {    
    ...
    case FlagType.recomended:
      return <RecomendedBlip {...props} config={config}></RecomendedBlip>;
    default:
      return <DefaultBlip {...props} config={config} />;
  }
}

const BlipPoints2: React.FC<{
  quadrantName: string;
  items: Item[];
  xScale: ScaleLinear<number, number>;
  yScale: ScaleLinear<number, number>;
  config: ConfigData;
}> = ({ quadrantName, items, xScale, yScale, config }) => {
  const blips: Blip[] = items.reduce((list: Blip[], item: Item) => {
    // skip the blip if it doesn't have a ring or quadrant assigned or not in this quadrant
    if (!item.ring || !item.quadrant || item.quadrant != quadrantName) {
      return list;
    }
