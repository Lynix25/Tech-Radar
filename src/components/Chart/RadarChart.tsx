  ...
  return (
    <div className="chart" style={{ maxWidth: `${config.chartConfig.size}px` }}>
      ...
      >
        ...
        <path style={{ fill: "rgb(0, 0, 0)" }} d={`M ${(config.chartConfig.size / 2) - lineSize / 2} 0 L ${(config.chartConfig.size / 2) - lineSize / 2} ${config.chartConfig.size} L ${(config.chartConfig.size / 2) + lineSize / 2} ${config.chartConfig.size} L ${(config.chartConfig.size / 2) + lineSize / 2} 0 L ${(config.chartConfig.size / 2) - lineSize / 2} 0 Z`}></path>
        <path style={{ fill: "rgb(0, 0, 0)" }} d={`M 0 ${(config.chartConfig.size / 2) + lineSize / 2} L ${config.chartConfig.size} ${(config.chartConfig.size / 2) + lineSize / 2} L ${config.chartConfig.size} ${(config.chartConfig.size / 2) - lineSize / 2} L 0 ${(config.chartConfig.size / 2) - lineSize / 2} L 0 ${(config.chartConfig.size / 2) + lineSize / 2} Z`}></path>
